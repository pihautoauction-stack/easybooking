    ...
    useEffect(() => {
        ...
        const init = async () => {
            ...
            if (!tg?.initData) {
                ...
                setLoading(false);
                return; // В браузере дальше не идем
            }

            // 2. МЫ В TELEGRAM: Настройка UI
            tg.ready();
            tg.expand();
            if (tg.setHeaderColor) tg.setHeaderColor('#0f172a');

            // 3. ОБРАБОТКА ТОКЕНА ВХОДА (startapp)
    ...
    if (isBrowser) {
        return (
            <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 text-center text-white font-sans">
                ...
                <p className="text-slate-400 mb-8">Для управления кабинетом перейдите в Telegram.</p>
                {returnLink && (
                ...
                <button onClick={() => supabase.auth.signOut().then(() => router.push("/login"))} className="mt-8 text-slate-500 text-sm underline">Выйти</button>
            </div>
        );
    }

    // ОСНОВНОЙ КАБИНЕТ (Только в Telegram)
    return (
        <div className="min-h-screen bg-slate-900 text-white p-4 pb-20 font-sans">
            ...
            <main className="grid gap-6">
                ...
                <div className="bg-gradient-to-br from-blue-900/40 to-slate-800 p-5 rounded-2xl border border-blue-500/30">
                    <h2 className="text-xs font-bold uppercase text-blue-300 mb-3 tracking-wider flex items-center gap-2"><LinkIcon className="w-3 h-3" /> Ваша ссылка</h2>
                ...
                <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-md">
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={handleSaveProfile} disabled={saving} className="w-full bg-blue-600 py-4 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg transition-all">{saving ? <Loader2 className="w-5 h-5 animate-spin" /> : "Сохранить настройки"}</button>
                    </div>
                </div>
                    ...
                    <div className="space-y-2">
                        {services.map(s => (
                            <div key={s.id} className="flex justify-between items-center bg-slate-700/30 p-3 rounded-xl border border-slate-600/50">
                                ...
                                <button onClick={() => handleDeleteRecord(s.id)} className="text-slate-500 hover:text-red-400 p-2 transition-colors"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        ))}
                    </div>
                </div>
