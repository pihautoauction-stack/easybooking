...
export default function Dashboard() {
    ...
    useEffect(() => {
        ...
        const init = async () => {
            setLoading(true);
            setDebugInfo("Запуск инициализации...");

            // 1. ПРОВЕРКА СРЕДЫ
            if (!tg?.initData) {
                setIsBrowser(true);
                setDebugInfo(prev => prev + "\nСреда: Safari/Браузер");
            ...
            if (startParam && startParam.length > 30) {
                setDebugInfo(prev => prev + "\nНайден токен, входим...");
                const { data: refreshData, error: refreshError } = await supabase.auth.refreshSession({ refresh_token: startParam });
                if (refreshError) {
                    setDebugInfo(prev => prev + "\nОшибка токена: " + refreshError.message);
                } else if (refreshData.session) {
                    setDebugInfo(prev => prev + "\nВход по токену успешен");
            ...
            if (!authUser) {
                setDebugInfo(prev => prev + "\nСессия не найдена, кидаю на login");
                router.push("/login");
                return;
            }
        ...
        <div className="min-h-screen bg-slate-900 text-white p-4 pb-20 font-sans">
            ...
            <div className="mb-4 p-3 bg-black/50 border border-yellow-500/50 rounded-xl text-[10px] font-mono text-yellow-200">
                <div className="flex items-center gap-2 mb-1 border-b border-yellow-500/30 pb-1">
                    <Bug className="w-3 h-3" /> DEBUG STATUS
                </div>
            ...
            {isBrowser ? (
                <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
                    ...
                    <p className="text-slate-400 mb-8 max-w-xs">Нажмите кнопку, чтобы управлять записями через Telegram.</p>
                    {returnLink && (
                    ...
                    <main className="grid gap-6">
                        ...
                        <div className="bg-slate-800 p-5 rounded-2xl border border-slate-700 shadow-md">
                            ...
                            <div className="space-y-3">
                                {appointments.length === 0 ? <p className="text-slate-500 text-center py-4 text-sm">Пусто</p> : appointments.map(app => (
