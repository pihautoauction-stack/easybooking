...
export default function BookingPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleBooking = async (e: React.FormEvent) => {
        ...
        const res = await fetch('/api/notify', {
            ...
            body: JSON.stringify({ masterId: profile.id, serviceId: selectedService.id, clientName, clientPhone, startTime }),
        });
    ...
    if (bookingStatus === "success") {
        return (
            <div className="min-h-screen bg-[#050505] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(16,185,129,0.15),rgba(255,255,255,0))] flex flex-col items-center justify-center text-white p-4 sm:p-6 text-center font-sans">
                ...
                <button onClick={() => window.location.reload()} className="w-full max-w-xs mx-auto bg-white text-black font-bold py-3.5 sm:py-4 rounded-xl sm:rounded-2xl shadow-[0_0_20px_rgba(255,255,255,0.2)] active:scale-95 text-sm sm:text-base">Новая запись</button>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-[#050505] bg-[radial-gradient(ellipse_80%_80%_at_50%_-20%,rgba(37,99,235,0.15),rgba(255,255,255,0))] text-white p-4 sm:p-5 font-sans pb-24 selection:bg-blue-500/30">
            <div className="max-w-md mx-auto w-full">
                <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 pt-2 sm:pt-4">
                    {selectedService && (
                        <button onClick={() => setSelectedService(null)} className="p-2 sm:p-3 bg-white/5 backdrop-blur-md rounded-xl sm:rounded-2xl border border-white/10 active:scale-95 shrink-0"><ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" /></button>
                    )}
                ...
                {!selectedService ? (
                    <div className="space-y-3 sm:space-y-4">
                        ...
                        {services.map((service) => (
                            <div key={service.id} onClick={() => setSelectedService(service)} className="bg-white/[0.03] backdrop-blur-xl rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 border border-white/10 active:scale-[0.98] shadow-lg relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-20 h-20 sm:w-24 sm:h-24 bg-blue-500/5 rounded-full blur-2xl -z-10"></div>
                    ...
                    <div className="space-y-4 sm:space-y-6">
                        ...
                        <div className="flex justify-center bg-black/40 rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 border border-white/5 shadow-inner backdrop-blur-md overflow-x-auto w-full">
                            <style>{`
                                /* ГЕНИАЛЬНЫЙ CSS ХАК ДЛЯ МОБИЛОК: Размер ячейки зависит от ширины экрана, но не больше 42px */
                        ...
                        {selectedDate && (
                            <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 sm:gap-3 pt-1 sm:pt-2">
                                {availableSlots.length > 0 ? availableSlots.map(t => (
                                    <button key={t} onClick={() => setSelectedTime(t)} className={`py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl text-xs sm:text-sm font-bold transition-all ${selectedTime === t ? "bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.4)] border border-blue-400/30 text-white" : "bg-white/5 border border-white/10 text-white/60"}`}>{t}</button>
                            ...
                            <form onSubmit={handleBooking} className="space-y-3 sm:space-y-4 pt-4 sm:pt-6 mt-2 sm:mt-4 border-t border-white/10">
                                <div className="space-y-2.5 sm:space-y-3">
                                    <div className="relative">
                                        ...
                                        <input required value={clientName} onChange={e => setClientName(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl sm:rounded-2xl py-3 sm:py-4 pl-10 sm:pl-12 pr-4 text-xs sm:text-sm text-white outline-none focus:border-blue-500/50" placeholder="Ваше имя" />
                                    </div>
                                    <div className="relative">
                                        <Phone className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-white/30" />
                                        <input required type="tel" value={clientPhone} onChange={e => setClientPhone(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-xl sm:rounded-2xl py-3 sm:py-4 pl-10 sm:pl-12 pr-4 text-xs sm:text-sm text-white outline-none focus:border-blue-500/50 font-mono" placeholder="+7 (999) 000-00-00" />
                                    </div>
                                </div>
                                <button type="submit" disabled={bookingStatus === "submitting"} className="w-full bg-white text-black font-bold py-3.5 sm:py-4 rounded-xl sm:rounded-2xl shadow-[0_0_20px_rgba(255,255,255,0.2)] active:scale-95 text-sm sm:text-base flex items-center justify-center gap-2 mt-2 sm:mt-4">
                                    {bookingStatus === "submitting" ? <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" /> : "Подтвердить запись"}
                                </button>
                            </form>
                        )}
                        
                        {bookingStatus === "conflict" && <div className="bg-orange-500/10 text-orange-300 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center text-xs sm:text-sm border border-orange-500/20 animate-pulse mt-3 sm:mt-4">Упс! Это время только что заняли.</div>}
                        {bookingStatus === "error" && <div className="bg-red-500/10 text-red-300 p-3 sm:p-4 rounded-xl sm:rounded-2xl text-center text-xs sm:text-sm border border-red-500/20 mt-3 sm:mt-4">Ошибка сервера. Попробуйте позже.</div>}
                    </div>
                )}
            </div>
        </div>
